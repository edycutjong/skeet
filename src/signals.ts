...
export class Signals {
  ...
  public update(price: number, volume: number = 0) {
    ...
    this.state.emaFast =
      price * alphaFast + this.state.emaFast * (1 - alphaFast);
    this.state.emaSlow =
      price * alphaSlow + this.state.emaSlow * (1 - alphaSlow);
  ...
  // Reversal Detection: if price drops from peak by configured trail % or EMA fast crosses below EMA slow (after stabilization)
  public isReversalDetected(reversalTrailPct: number = -0.03): boolean {
    if (this.peakPrice <= 0) return false;
    const dropPct = (this.lastPrice - this.peakPrice) / this.peakPrice;
    if (dropPct <= reversalTrailPct) return true;
    if (this.state.tickCount > 5 && this.state.emaFast < this.state.emaSlow)
      return true;
    return false;
  }
  ...
  private computeRealizedVol(): number {
    ...
    const mean =
      this.logReturns.reduce((sum, val) => sum + val, 0) /
      this.logReturns.length;
    const variance =
      this.logReturns.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) /
      (this.logReturns.length - 1);
