...
async function api(
  pathStr: string,
  {
    ...
    token,
    body,
  }: { method?: string; token?: string; body?: any } = {},
) {
  ...
  const r = await fetch(API_ENDPOINT + pathStr, {
    method,
    headers,
  ...
  if (!r.ok) {
    const e = new Error(
      `${method} ${pathStr} → ${r.status}: ${data.error || text}`,
    ) as any;
    e.status = r.status;
    throw e;
  }
...
async function loadOrBootstrap(accessCode: string): Promise<AgentState> {
  ...
  console.log(
    "[INDEX] Bootstrapping fresh agent wallet and registering with Creatorbid...",
  );
  ...
  if (!registrationRes.trading_safe) {
    throw new Error(
      "Registration failed: no trading safe provisioned. Response: " +
        JSON.stringify(registrationRes),
    );
  }
  ...
  console.log(
    `[INDEX] Agent registered successfully! Trading Safe: ${state.tradingSafe}`,
  );
  return state;
}
