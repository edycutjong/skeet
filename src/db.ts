...
export function getRounds(db: Database.Database): RoundRow[] {
  return db
    .prepare("SELECT * FROM rounds ORDER BY ts DESC")
    .all() as RoundRow[];
}

export function getTicks(db: Database.Database, gameId: string): TickRow[] {
  return db
    .prepare("SELECT * FROM ticks WHERE game_id = ? ORDER BY t ASC")
    .all(gameId) as TickRow[];
}
...
export function getDailyPnL(
  ...
  const row = db
    .prepare("SELECT SUM(pnl_usdc) as total_pnl FROM rounds WHERE ts >= ?")
    .get(sinceTimestamp) as { total_pnl: number | null };
