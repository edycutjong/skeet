...
export async function runFeed(
  ...
  const startHeartbeat = (address: string) => {
    const ping = () =>
      fetch("https://alpha.creator.bid/api/agents/heartbeat", {
        method: "POST",
  ...
  const checkAndRefill = async (address: string) => {
    try {
      ...
      if (ethBal < ethers.parseEther("0.1")) {
        console.log(
          `[FEED] Balance low (ETH: ${ethers.formatEther(ethBal)}). Requesting refill...`,
        );
  ...
  let enteredThisRound = false;
  let exitedThisRound = false;
  let lastSettledTotalUsdc: number | null = null;
  let lastRoundIdForPnl = "";
  let entryUsdcThisRound = 0;
  ...
  while (true) {
    ...
    if (isNewBattle) {
      console.log(
        `[FEED] New battle detected: ${game.token.name} (${game.token.symbol})`,
      );
    ...
    try {
      ...
      tokBal = await new ethers.Contract(
        tokenAddress,
        ERC20_ABI,
        provider,
      ).balanceOf(state.tradingSafe);
    } catch (e: any) {
    ...
    // Settle realized PnL on round changes using boundary total USDC balances
    if (currentRoundId !== lastRoundIdForPnl) {
      if (lastRoundIdForPnl !== "" && lastSettledTotalUsdc !== null) {
        const realizedPnl = usdcBalNum - lastSettledTotalUsdc;
        console.log(
          `[PNL] Settle round ${lastRoundIdForPnl}: realized PnL = ${realizedPnl.toFixed(2)} USDC`,
        );
        db.prepare("UPDATE rounds SET pnl_usdc = ? WHERE game_id = ?").run(
          realizedPnl,
          lastRoundIdForPnl,
        );
      }
      lastSettledTotalUsdc = usdcBalNum;
      lastRoundIdForPnl = currentRoundId;
      console.log(
        `[PNL] cumulative ≈ ${(usdcBalNum - config.START_BANKROLL).toFixed(2)} USDC (total ${usdcBalNum.toFixed(2)})`,
      );
    }
    ...
    if (config.PREDATOR_ENABLED) {
      try {
        const resTrades = await fetch(
          `https://alpha.creator.bid/api/tokens/${tokenAddress}/trades`,
        );
        if (resTrades.ok) {
    ...
    console.log(
      `[FEED] Tick t=${t.toFixed(1)} | Price: ${price.toFixed(4)} | Safe: ${parseFloat(ethers.formatUnits(tradingUsdc, 18)).toFixed(2)} USDC | Treasury: ${parseFloat(ethers.formatUnits(treasuryUsdc, 18)).toFixed(2)} USDC | Bankroll: ${currentBankroll.toFixed(2)} USDC | Pool Reserves: ${reservesUsdc.toFixed(2)} USDC | Action: ${action.type}`,
    );
    ...
    if (dailyPnL <= -config.MAX_DAILY_LOSS_USDC) {
      if (action.type === "BUY") {
        console.warn(
          `[CIRCUIT BREAKER] BUY blocked: 24h loss of ${dailyPnL.toFixed(2)} USDC exceeds limit of -${config.MAX_DAILY_LOSS_USDC} USDC`,
        );
        action.type = "HOLD";
        action.amount = 0;
      }
    ...
    const canTrade =
      (action.type === "BUY" && !enteredThisRound) ||
      (action.type === "SELL_ALL" && !exitedThisRound) ||
      (action.type === "SELL_PARTIAL" && !exitedThisRound);
      ...
      (async () => {
        try {
          if (action.type === "BUY" && action.amount) {
            const buyAmountWei = ethers.parseUnits(
              action.amount.toFixed(6),
              18,
            );
            console.log(
              `[FEED] BUYing ${action.amount} USDC worth of ${game.token.symbol}`,
            );
            enteredThisRound = true;
            entryUsdcThisRound = action.amount;
            ...
            console.log(
              `[FEED] SELLING ALL ${ethers.formatUnits(tokBal, 18)} tokens of ${game.token.symbol}`,
            );
            exitedThisRound = true;
            ...
            const sellAmount =
              (tokBal * BigInt(Math.floor(action.amount * 100))) / 100n;
            console.log(
              `[FEED] SELLING PARTIAL ${ethers.formatUnits(sellAmount, 18)} tokens of ${game.token.symbol}`,
            );
