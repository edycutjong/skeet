...
export class Executor {
  ...
  constructor(
    ...
    this.state = state;
    this.provider = new ethers.JsonRpcProvider(rpcUrl, 42069, { staticNetwork: true });
    this.wallet = new ethers.Wallet(state.pk, this.provider);
    this.rolesContract = new ethers.Contract(state.rolesMod, ROLES_ABI, this.wallet);
  ...
  private async siweLogin(): Promise<string> {
    ...
    if (!resNonce.ok) throw new Error(`SIWE nonce error: ${resNonce.statusText}`);
    ...
    if (!resLogin.ok) throw new Error(`SIWE login error: ${resLogin.statusText}`);
   ...
   */
  private async executeTx(calldata: string, operation: number = 1): Promise<any> {
    const next = this.transactionQueue.catch(() => {}).then(async () => {
   ...
   */
  public async getSwapSignature(tokenAddress: string, amountIn: bigint, isBuy: boolean): Promise<any> {
  ...
  public async approveToken(tokenAddress: string): Promise<any> {
    const data = this.traderInterface.encodeFunctionData('approveFactory', [tokenAddress, ethers.MaxUint256]);
   ...
   */
  public async executeSwap(tokenAddress: string, amountIn: bigint, isBuy: boolean): Promise<any> {
