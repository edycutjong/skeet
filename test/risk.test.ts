...
describe('Risk engine calculations', () => {
  ...
  test('Kelly Fraction calculations', () => {
    ...
    expect(kellyFraction(0.60, 1.0)).toBeCloseTo(0.2, 5); // 0.60 * 2 - 1 = 0.2
    expect(kellyFraction(0.40, 1.0)).toBe(0); // negative clamped to 0
  ...
  test.each(bankrollScaleCases)('Sizing monotonicity with bankroll %#', ({ bankroll, expectedSize }) => {
  ...
  test.each(capCases)('Cap enforcement for large bankrolls %#', ({ bankroll, peak, expectedSize }) => {
  ...
  test('Probe mode threshold limits sizes when bankroll is below floor', () => {
    ...
    const sizeCapped = kellySize(0.80, 2000, 2000, mockConfig);
