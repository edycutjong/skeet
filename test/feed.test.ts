...
const mockPrepareRun = vi.fn().mockReturnValue({ changes: 1 });
const mockPrepare = vi.fn().mockReturnValue({ run: mockPrepareRun });
...
vi.mock("../src/decide.js", () => ({
  decide: (
    ...
    config: any,
    winRate: number,
    predatorAction?: string,
  ) => mockDecide(ctx, stats, config, winRate, predatorAction),
}));
...
describe("runFeed game loop orchestrator tests", () => {
  ...
  beforeEach(() => {
    ...
    mockExecuteSwap.mockResolvedValue("swap_receipt");
    mockApproveToken.mockResolvedValue("approve_receipt");
  });
  ...
  test("Handles contract call errors and trade fetch errors gracefully", async () => {
    ...
    mockProviderGetBalance.mockRejectedValueOnce(
      new Error("Low level getBalance fail"),
    );
  ...
  test("enteredThisRound blocks duplicate BUYs and exitedThisRound blocks subsequent actions", async () => {
    let gameCalls = 0;
    globalFetchMock.mockImplementation((url: string) => {
      if (url.endsWith("/api/game")) {
        gameCalls++;
    ...
    // Mock decide to return:
    // Call 1 -> BUY (triggers swap, sets enteredThisRound = true)
    // Call 2 -> BUY (ignored because enteredThisRound is true)
    // Call 3 -> SELL_ALL (triggers swap, sets exitedThisRound = true)
    // Call 4 -> SELL_ALL (ignored because exitedThisRound is true)
    mockDecide
      .mockReturnValueOnce({ type: "BUY", amount: 100 })
      .mockReturnValueOnce({ type: "BUY", amount: 100 })
      .mockReturnValueOnce({ type: "SELL_ALL" })
      .mockReturnValueOnce({ type: "SELL_ALL" });

    // Mock token balance so we have token balance for sell
    mockBalanceOf.mockResolvedValue(100n * 10n ** 18n);
    ...
    // mockExecuteSwap should only be called 2 times (1 for BUY, 1 for SELL_ALL)
    expect(mockExecuteSwap).toHaveBeenCalledTimes(2);
  });

  test("Real boundary-based PnL settlement", async () => {
    let gameCalls = 0;
    globalFetchMock.mockImplementation((url: string) => {
      if (url.endsWith("/api/game")) {
        gameCalls++;
    ...
    mockBalanceOf.mockImplementation(() => {
      balanceCalls++;
      if (balanceCalls <= 2) {
        return Promise.resolve(100n * 10n ** 18n);
      } else {
        return Promise.resolve(80n * 10n ** 18n);
      }
    });
    ...
    expect(mockPrepare).toHaveBeenCalledWith(
      "UPDATE rounds SET pnl_usdc = ? WHERE game_id = ?",
    );
    expect(mockPrepareRun).toHaveBeenCalledWith(-40, "0xTokenA_1000");
  });

  test("preserves buy_usdc on SELL_ALL", async () => {
    let gameCalls = 0;
    globalFetchMock.mockImplementation((url: string) => {
      if (url.endsWith("/api/game")) {
        gameCalls++;
    ...
    mockDecide
      .mockReturnValueOnce({ type: "BUY", amount: 500 })
      .mockReturnValueOnce({ type: "HOLD" })
      .mockReturnValueOnce({ type: "SELL_ALL" })
      .mockReturnValueOnce({ type: "HOLD" })
      .mockReturnValueOnce({ type: "HOLD" });
    ...
    const sellAllSaveCall = mockSaveRound.mock.calls.find(
      (c) => c[1].exit_t > 0,
    );
    expect(sellAllSaveCall).toBeDefined();
    expect(sellAllSaveCall[1].buy_usdc).toBe(500);
    expect(sellAllSaveCall[1].pnl_usdc).toBe(0);
  });
});
