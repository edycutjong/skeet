...
describe("Signals statistics engine tests", () => {
  ...
  test("First and second ticks EMA behavior under discard rule", () => {
    const sigs = new Signals(5, 20);
    sigs.update(99); // first tick (discarded)
    expect(sigs.getEmaFast()).toBe(0);
    expect(sigs.getEmaSlow()).toBe(0);

    sigs.update(100); // second tick (seeds)
    expect(sigs.getEmaFast()).toBe(100);
    expect(sigs.getEmaSlow()).toBe(100);
    expect(sigs.getRealizedVol()).toBe(0); // not computed on early return

    sigs.update(101); // third tick (computed)
  ...
  test.each(priceCases)(
    "EMA fast/slow crossovers over price series %#",
    ({ prices, expectedFast, expectedSlow }) => {
      const sigs = new Signals(5, 20);
      sigs.update(100); // prepend stale tick (discarded)
  ...
  test.each(volCases)(
    "Volatility calculations for series %#",
    ({ prices, expectedVol }) => {
      const sigs = new Signals(5, 20);
      for (const p of prices) {
        sigs.update(p);
  ...
  test("Edge cases for signals update", () => {
    ...
    sigs.update(100); // 1st tick (discarded)
    expect(sigs.getTickCount()).toBe(1);

    sigs.update(100); // 2nd tick (seeded)
    expect(sigs.getRealizedVol()).toBe(0);

    sigs.update(100); // 3rd tick (computed)
  ...
  test("Volume rising indicator behavior", () => {
    ...
    sigs.update(100, 0); // 1 discarded
    sigs.update(100, 0); // 2 seeded
    sigs.update(100, 10);
    sigs.update(100, 12);
    sigs.update(100, 15);
    sigs.update(100, 18);
    ...
    sigs2.update(100, 0); // 1 discarded
    sigs2.update(100, 0); // 2 seeded
    sigs2.update(100, 20);
    sigs2.update(100, 18);
    sigs2.update(100, 12);
    sigs2.update(100, 8);
  ...
  test("Reversal detection", () => {
    const sigs = new Signals(5, 20);
    sigs.update(100); // first tick (discarded)
    sigs.resetPeakPrice();
    expect(sigs.isReversalDetected()).toBe(false);

    // Small drop (1%) -> no reversal
    sigs.update(99); // second tick (seeds peak to 99)
    expect(sigs.isReversalDetected()).toBe(false);

    // Large drop (4%) -> reversal (from peak of 99 to 96 is >3% drop)
    sigs.update(96);
    expect(sigs.isReversalDetected()).toBe(true);
  });

  test("Customizable reversal trail percent", () => {
    const sigs = new Signals(5, 20);
    sigs.update(100); // first tick (discarded)
    sigs.update(100); // second tick (seeded)
    sigs.resetPeakPrice();

    // 4% drop -> false under -6% trail
    sigs.update(96);
    expect(sigs.isReversalDetected(-0.06)).toBe(false);

    // 7% drop -> true under -6% trail
    sigs.update(93);
    expect(sigs.isReversalDetected(-0.06)).toBe(true);
  });
