...
const mockExecTransactionWithRole = vi.fn().mockResolvedValue({ wait: mockWait });
...
describe('Executor class test suite', () => {
  ...
  test('siweLogin throws on nonce failure', async () => {
    ...
    const executor = new Executor(agentState);
    await expect(executor.getFreshJwt()).rejects.toThrow('SIWE nonce error: Internal Server Error');
  });
  ...
  test('siweLogin throws on login failure', async () => {
    ...
    const executor = new Executor(agentState);
    await expect(executor.getFreshJwt()).rejects.toThrow('SIWE login error: Unauthorized');
  });
  ...
  test('getSwapSignature throws on API failure', async () => {
    ...
    const executor = new Executor(agentState);
    await expect(executor.getSwapSignature('0xTokenAddress', 1000n, true)).rejects.toThrow(
      'Swap signature API error 400: Invalid params'
    );
  });
  ...
  test('executeTx fails after 2 attempts', async () => {
    ...
    mockExecTransactionWithRole.mockRejectedValue(new Error('Permanent failure'));

    const executor = new Executor(agentState);
    await expect(executor.approveToken('0xTokenAddress')).rejects.toThrow('Permanent failure');
    expect(mockExecTransactionWithRole).toHaveBeenCalledTimes(2);
  });
