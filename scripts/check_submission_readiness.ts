...
function checkReadiness() {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const regex of placeholders) {
        if (regex.test(line)) {
          ...
          if (
            file === "scripts/check_submission_readiness.ts" ||
            line.includes("checkReadiness")
          )
            continue;

          console.error(
            `❌ Placeholder violation in ${file}:${(i + 1).toString()}`,
          );
  ...
  if (clean) {
    ...
    console.error(
      "❌ READINESS CHECK FAILED: Please replace all placeholders.",
    );
    process.exit(1);
  }
  console.log("==================================================");
