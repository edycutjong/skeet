...
class SimulatedMeanReversionAgent {
  ...
  public execute(price: number, stats: Signals, t: number) {
    ...
    this.peakBankroll = Math.max(this.peakBankroll, this.bankroll + this.position * price);
  }
}
...
class SimulatedSkeetAgent {
  ...
  public execute(price: number, stats: Signals, t: number) {
    ...
    if (action.type === "BUY" && action.amount) {
      ...
      this.position = 0;
      this.entryPrice = 0;
    } else if (action.type === "SELL_PARTIAL" && this.position > 0 && action.amount) {
...
function runBacktest() {
  ...
  for (let r = 1; r <= roundsCount; r++) {
    ...
    console.log(`Round ${r.toString().padStart(2, '0')} | Skeet PnL: ${skeetPnL.toFixed(2).padStart(8)} USDC | MeanReversion PnL: ${mrPnL.toFixed(2).padStart(8)} USDC`);
  }

  console.log('--------------------------------------------------');
  console.log(`TOTALS   | Skeet PnL: ${totalSkeetPnL.toFixed(2).padStart(8)} USDC | MeanReversion PnL: ${totalMeanReversionPnL.toFixed(2).padStart(8)} USDC`);
  console.log('==================================================');
  
  if (totalSkeetPnL > totalMeanReversionPnL) {
    console.log('✅ BACKTEST PASSED: Skeet outperforms MeanReversion!');
  } else {
    console.warn('⚠️ BACKTEST WARNING: Skeet did not outperform MeanReversion.');
  }
  console.log('==================================================');
}
