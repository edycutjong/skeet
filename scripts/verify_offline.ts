...
function runOfflineVerification() {
  ...
  for (let t = 0; t < pricePath.length; t++) {
    ...
    if (action.type === "BUY" && action.amount) {
      if (action.amount > mockConfig.MAX_BUYIN_USDC) {
        console.error(`❌ SAFETY VIOLATION: Position size ${action.amount} exceeds limit of ${mockConfig.MAX_BUYIN_USDC}`);
        process.exit(1);
      }
      ...
      entryPrice = price;
      didBuy = true;
      console.log(`[t=${t}] Executed BUY of ${action.amount.toFixed(2)} USDC at price ${price.toFixed(2)}`);
      ...
      didSell = true;
      exitSecond = t - 30;
      console.log(`[t=${t}] Executed SELL_ALL (Value: ${sellVal.toFixed(2)} USDC) at price ${price.toFixed(2)}`);
    }
  ...
  if (didBuy && !didSell) {
    console.error('❌ SAFETY VIOLATION: Position was not closed before game ended!');
    process.exit(1);
  }

  if (exitSecond !== -1 && exitSecond > mockConfig.EXIT_DEADLINE_S) {
    console.error(`❌ SAFETY VIOLATION: Position closed at second ${exitSecond}, past deadline ${mockConfig.EXIT_DEADLINE_S}`);
    process.exit(1);
  }

  console.log('--------------------------------------------------');
  console.log('✅ verification passed:');
  console.log(`   - Exit deadline respected: closed at second ${exitSecond} (limit: ${mockConfig.EXIT_DEADLINE_S})`);
  console.log('   - Position sizing capped under 1000 USDC');
  console.log(`   - Bankroll remained positive (Final: ${bankroll.toFixed(2)} USDC)`);
  console.log('==================================================');
}
